...
def get_offworkers(date):
    ...
    xlfile = pd.ExcelFile(copiedfile)
    #df = xlfile.parse( skiprows=range(1,17000), usecols='A:K')
    #A value is trying to be set on a copy of a slice from a DataFrame
    #df = pd.read_excel(excelfile, sheet_name= '민원처리내역',index_col=False, skiprows =  range(1,17000) , usecols='A:K' )
    # NOTE clean data frame
    #print( help(xlfile.parse) )
    #wb = load_workbook( copiedfile, data_only=True,read_only=True )
    #wb = load_workbook( xlfile, data_only=True,read_only=True )
    df = xlfile.parse( sheet_name='조별연차표',skiprows=[0,1,24,25,26,27], usecols='B:H')
    ...
    #ws = wb['조별연차표'] 
    def R1C1(row,col): return df.iloc[row,col-2]
    class team: day,night = get_workteam(date)
    class off: day,night = '',''
    from itertools import product
    rc = product([0,4,8,12,16],[2,3,4,5,6,7,8])
    for r,c in rc:
        print((r,c),R1C1(r,c),date.day)
        if R1C1(r,c) == date.day:
            off.day   = R1C1(r+1,c)
            off.night = R1C1(r+2,c)
            break
    else:
        raise Exception('invalid')

    #r,c  = [(r,c) for r,c in rc if R1C1(r,c) == date.day][0]
    #if R1C1(r+1,c).value: off.day   = R1C1(r+1,c).value
    #if R1C1(r+2,c).value: off.night = R1C1(r+2,c).value
    #wb.close()
    print(team.day,off.day)
...
if __name__ == '__main__':
    date = datetime.strptime('22-05-01','%y-%m-%d')
