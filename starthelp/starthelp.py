...
def get_weeklyreports(date):
    ...
    for day in range(start,end+1):
        ...
        if get_weekday(adate) == '목':
                weeklydates.append(adate)
    #print('weeklydatas =>',weeklydates)
    return weeklydates
...
def get_offworkers(date):
    ...
    wb = load_workbook( get_monthlycommutefile(date), data_only=True)
    #wb = load_workbook( get_monthlycommutefile(date), data_only=True,read_only=True )
    ws = wb['조별연차표'] 
    def R1C1(row,col): return ws.cell(row,col)
    class team: day,night = get_workteam(date)
    class off: day,night = '',''
    from itertools import product
    rc = product([4,8,12,16,20],[2,3,4,5,6,7,8])
    r,c  = [(r,c) for r,c in rc if R1C1(r,c).value == date.day][0]
